...
app.get('*', (req, res) => {
  ...
  // or, if query param to force landing page
  const loggedOutRootRequest = (req.path === '' || req.path === '/') && !req.user;
  const forceLanding = req.query.forceLanding === 'true';
  if (loggedOutRootRequest || forceLanding) {
